...
module.exports.execute = async(client , message, args) => {
    ...
    if (!member) return handler.mesaj("true", message.author, message.channel, `${answer.hatalikullanim} \`Örnek: ${config.client.prefix || '.'}nerede @Richârd/ID\``, 5000);
    if(!member.voice.channel) return handler.hata("true", message.author, message.channel, answer.seslidegil, 5000);
...
handler.mesaj("true", message.author, message.channel, `${emojis.onay} Selam \`${message.member.displayName}\`, İstediğin  ${member} üyesinin ses bilgileri aşağıda belirtilmiştir.
\`\`\`diff
